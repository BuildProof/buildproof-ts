...
interface VotingPageViewProps {
  ...
  ethPrice: string
  onRedeemClick: (claimId: string, maxStake: number) => void
}
            ...
            {sortedItems.map((item) => (
              <div key={item.id} className="relative">
                <ClaimRow
                  ...
                  userPosition={
                    item.userPosition &&
                    item.vault?.current_share_price &&
                    item.positionDirection === ClaimPosition.claimFor
                      ? formatTVL(
                          calculateStakeValue(
                            item.userPosition,
                            item.vault.current_share_price,
                      ...
                      : item.userPosition &&
                          item.counter_vault?.current_share_price &&
                          item.positionDirection === ClaimPosition.claimAgainst
                        ? formatTVL(
                            calculateStakeValue(
                              item.userPosition,
                              item.counter_vault.current_share_price,
                  ...
                  onStakeForClick={() =>
                    console.log('Vote FOR project', item.subject)
                  }
                  onStakeAgainstClick={() =>
                    console.log('Vote AGAINST project', item.subject)
                  }
                >
                      ...
                      {item.userPosition && (
                        <Button
                          ...
                          onClick={() => {
                            ...
                            const triple = triplesData?.triples?.find(
                              (t: {
                                id: string
                                vault_id: string
                        ...
                        <MultiSlider
                          slider={{
                            ...
                            onChangeEnd: (value: number) =>
                              handleSliderCommit(item.id, value),
                          }}
                        />
          ...
          <div className="max-w-4xl mx-auto">
            <div className="flex justify-center mb-4">
              ...
              >
                {canSubmit
                  ? 'Submit Vote'
                  : `${totalAbsoluteValue}% of 100% Allocated`}
              </Button>
            </div>
