...
import { Button, ButtonSize, ButtonVariant } from '@0xintuition/buildproof_ui'
  ...
  const handleConfirm = async () => {
    try {
      ...
      if (!walletClient.account) {
        throw new Error(
          'No wallet account selected. Please select an account in your wallet.',
        )
      }
      ...
      if (!env.PINATA_JWT) {
        throw new Error(
          'PINATA_JWT is not configured. Please contact the administrator.',
        )
      }
      ...
      const { value: titleValue, ipfsHash: titleIpfsHash } =
        await hashDataToIPFS(hackathonTitle, env.PINATA_JWT)
      const { value: descValue, ipfsHash: descIpfsHash } = await hashDataToIPFS(
        description,
        env.PINATA_JWT,
      )
      ...
      const filteredMissingAtoms = missingAtoms.filter(
      ...
      if (filteredMissingAtoms.length > 0) {
        ...
        try {
          ...
          throw new Error(
            'Failed to create atoms. Please check your wallet and try again.',
          )
        }
      }
      ...
      if (atomIds.some((id) => id === null)) {
        throw new Error(
          'Failed to create or retrieve required atoms. Please try again.',
        )
      }
      ...
      try {
        ...
        throw new Error(
          'Failed to create triples. Please check your wallet and try again.',
        )
      }
